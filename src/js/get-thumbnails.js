    ...
    .then(response => {
        parser.parseStringPromise(response.data).then(result => {
            ...
            entries.forEach((entry, index) => {
                ...
                videos += `<td style="padding: 8px; text-align: center;"><a href="https://www.youtube.com/watch?v=${videoId}" target="_blank"><img src="${thumbnail}" alt="${title}" style="width:100%; border-radius: 5px;"><br>${title}</a></td>`;
                if ((index + 1) % 3 === 0) {
                    videos += '</tr><tr>';
                }
            });
